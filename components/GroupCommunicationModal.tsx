...
export const GroupCommunicationModal: React.FC<GroupCommunicationModalProps> = ({ isOpen, onClose, users, userType, onSend }) => {
  ...
  const handleSend = () => {
      ...
      setTimeout(() => {
        onSend({
            text: `Group message sent to ${users.length} ${userType.toLowerCase()}(s) via ${channel}.`,
            timestamp: Date.now(),
            icon: 'envelope',
        });
        setIsLoading(false);
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex justify-center items-center p-4">
      <div className="bg-white border border-[var(--border-color)] rounded-lg shadow-xl p-8 w-full max-w-2xl max-h-full overflow-y-auto">
        <h2 className="text-2xl font-bold mb-2 text-[var(--text-dark)]">Group Communication</h2>
        <p className="text-sm text-[var(--text-light)] mb-6">Send a message to {users.length} selected {userType.toLowerCase()}(s).</p>
        ...
        <div className="space-y-4">
            ...
            <div>
                 <label className="block text-sm font-medium text-[var(--text-light)]">Recipients</label>
                 <div className="mt-1 p-2 border border-gray-200 rounded-md bg-gray-50 h-20 overflow-y-auto text-sm text-[var(--text-dark)]">
            ...
            {generatedMessage && (
                <div>
                    ...
                    <textarea value={generatedMessage} onChange={(e) => setGeneratedMessage(e.target.value)} rows={5} className="mt-1 block w-full bg-white border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-[var(--primary)] focus:border-[var(--primary)]"></textarea>
                </div>
            )}
        </div>
